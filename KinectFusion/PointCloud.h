...
public:
	...
	PointCloud(float* depthMap, const Matrix3f& depthIntrinsics, const Matrix4f& depthExtrinsics, const unsigned & width, const unsigned & height, unsigned downsampleFactor = 1, float maxDistance = 0.1f) {
		...
		for (int v = 0; v < height; ++v) {
			...
			for (int u = 0; u < width; ++u) {
				...
				if (depth == MINF) {
					...
					pointsTmp[idx] = rotationInv * Vector3f((u - cX)*depth / fovX, (v - cY)*depth / fovY, depth) + translationInv;
				}
			}
		}
		...
		for (int v = 1; v < height - 1; ++v) {
			for (int u = 1; u < width - 1; ++u) {
				...
				if (!std::isfinite(du) || !std::isfinite(dv) || abs(du) > maxDistanceHalved || abs(dv) > maxDistanceHalved) {	// TODO: Check the abs shit
		...
		for (int i = 0; i < nPoints; i = i + downsampleFactor) {
			...
			m_raw_normals.push_back(normal);
		}
	...
	std::vector<Vector3f>& getRawNormals() {
		return m_raw_normals;
	}

	const std::vector<Vector3f>& getRawNormals() const {
		return m_raw_normals;
	}
...
private:
	...
	std::vector<Vector3f> m_raw_normals;
