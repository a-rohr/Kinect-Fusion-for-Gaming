...
public:
	...
	PointCloud(float* depthMap, const Matrix3f& depthIntrinsics, const Matrix4f& depthExtrinsics, const unsigned width, const unsigned height, unsigned downsampleFactor = 1, float maxDistance = 0.1f) {
		...
		for (int v = 1; v < height - 1; ++v) {
			for (int u = 1; u < width - 1; ++u) {
				...
				Vector3f diffU = pointsTmp[idx + 1] - pointsTmp[idx - 1];
				Vector3f diffV = pointsTmp[idx + width] - pointsTmp[idx - width];
				normalsTmp[idx] = diffU.cross(diffV);
	...
	PointCloud(std::vector<PointCloud> &pointClouds)
	{
		int numTotalPoints = 0;
		for (auto pointCloud : pointClouds)
		{
			numTotalPoints += pointCloud.getPoints().size();
		}
		m_points.reserve(numTotalPoints);
		m_normals.reserve(numTotalPoints);
		...
		for (auto pointCloud : pointClouds)
		{
			...
			for (auto normal : pointCloud.getNormals())
			{
				m_normals.push_back(normal);
			}
		}
