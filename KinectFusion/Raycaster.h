...
public:
		...
		Vector3d &vertex, Vector3f &normal, const double sizeVoxel)
	{
		...
		//While in absolute free space advance at steps of size miu
		while (current_Val > 0.99f) //(ideally ==1.0f)
		{
			...
			if (length > 8) { return false; }//The vision limit is 8m. V E R I F Y
			if (!vol.withinGrid(currentPos)) { return false; };
			//We don't need to use the "expensive" trilinear interp at this point
			current_Val = vol.getNodeValAtWorld(currentPos); //This is the rough voxel Vale

			//CONCERN: what if it advances too much and it gets out of volume. NOT POSSIBLE: If the
			//value is 1 it would to advance AT LEAST BY miu to cross the zero level
		}

		//Eval previous and current with trilinear
		...
		std::vector<Vector3f> &hitNormals, float miu)
	{
		...
		for (int y = 0; y < _depthImgHeight; y++)
		{
			...
			for (int x = 0; x < _depthImgWidth; x++)
			{
				...
				//zero_cross_finder(Volume &vol, Vector3f origin, Vector3f rayDir, float &length, float miu,
				//Vector3f &vertex, Vector3f &normal, const double sizeVoxel)
