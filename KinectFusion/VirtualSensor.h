...
public:
	...
	bool init(const std::string& datasetDir)
	{
		...
		printf("Depth size: %i, RGB size: %i.\n", m_filenameDepthImages.size(), m_filenameColorImages.size());
	...
	bool processNextFrameNoTxt(const std::string location)
	{
		if (m_currentIdx == -1)	m_currentIdx = 0;
		else m_currentIdx += m_increment;
		...
		FreeImageB rgbImage;
		int number = 0; 
		int copyIdx = m_currentIdx;
		do { copyIdx /= 10; number++; } while (copyIdx != 0);
		const std::string rgbName = location +"seq_color" + std::string(5-number, '0') + std::to_string(m_currentIdx) + ".png";
		//const char *rgbPrint = rgbName.c_str();
		//printf("RGB process: %s.\n", rgbPrint);
		rgbImage.LoadImageFromFile(rgbName);
		memcpy(m_colorFrame, rgbImage.data, 4 * 640 * 480);

		// depth images are scaled by 5000 (see https://vision.in.tum.de/data/datasets/rgbd-dataset/file_formats)
		FreeImageU16F dImage;
		const std::string depthName = location +"seq_depth" + std::string(5-number, '0') + std::to_string(m_currentIdx) + ".png";
		//const char *depthPrint = depthName.c_str();
		//printf("Depth process: %s.\n", depthPrint);
		dImage.LoadImageFromFile(depthName);
