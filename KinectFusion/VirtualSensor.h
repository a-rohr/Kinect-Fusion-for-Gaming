...
public:
	...
	bool processNextFrame() {
		...
		std::cout << m_filenameDepthImages[m_currentIdx] << "    " << m_filenameColorImages[m_currentIdx]  << std::endl;
		...
		for (unsigned int i = 0; i < m_depthImageWidth * m_depthImageHeight; ++i) {
			...
			else
			{
				...
				//unsigned int v = i / m_depthImageWidth;
				//unsigned int u = i % m_depthImageWidth;

				/*if (310 < u && u < 330 && 230 < v && v < 250)
				{
					m_depthFrame[i] = 0.3f;
				}
				else
