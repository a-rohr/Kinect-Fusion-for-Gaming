...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		auto poseIncrement = PoseIncrement<T>(const_cast<T* const>(pose));

		T inpPt[3];
		fillVector<T>(m_sourcePoint, inpPt);

		T transformedPt[3];
		poseIncrement.apply(inpPt, transformedPt);

		residuals[0] = T(m_weight) * (T(m_targetPoint(0)) - transformedPt[0]);
		residuals[1] = T(m_weight) * (T(m_targetPoint(1)) - transformedPt[1]);
		residuals[2] = T(m_weight) * (T(m_targetPoint(2)) - transformedPt[2]);
...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		T res_one[3];
		res_one[0] = T(m_weight) * (T(m_targetPoint(0)) - transformedPt[0]);
		res_one[1] = T(m_weight) * (T(m_targetPoint(1)) - transformedPt[1]);
		res_one[2] = T(m_weight) * (T(m_targetPoint(2)) - transformedPt[2]);

		residuals[0] = T(m_targetNormal(0)) * res_one[0] + T(m_targetNormal(1)) * res_one[1] + T(m_targetNormal(2)) * res_one[2];
