...
public:
		...
		for (unsigned int v = 0; v < sensor.getDepthImageHeight(); ++v) {
			...
			for (unsigned int u = 0; u < sensor.getDepthImageWidth(); ++u) {
				...
				if (depth == MINF) {
					...
					m_vertices[idx].color = Vector4uc(255,255,255,255);
				}
			}
		}
	...
	bool writeMeshObj(const std::string& filename) {
		// Write off file.
		std::ofstream outFile(filename);
		if (!outFile.is_open()) return false;
		...
		// Save faces.
		for (unsigned int i = 0; i < m_triangles.size(); i++) {
			outFile << "f " << (m_triangles[i].idx0 + 1) << " " << (m_triangles[i].idx1 + 1) << " " << (m_triangles[i].idx2 + 1) << std::endl;
		}
