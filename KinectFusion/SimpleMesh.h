...
public:
	...
	SimpleMesh(std::vector<Vector3f> points, int depthImgHeight, int depthImgWidth, float edgeThreshold = 0.01f)
	{
		// NOT COMPLETED
		...
		for (int i = 0; i < points.size(); i++)
		{
			...
			m_vertices[i].color = Vector4uc(255, 255, 255, 255);
		}

		// Compute triangles (faces).
		m_triangles.reserve((depthImgHeight - 1) * (depthImgWidth - 1) * 2);
		for (unsigned int i = 0; i < depthImgHeight - 1; i++) {
			for (unsigned int j = 0; j < depthImgWidth - 1; j++) {
				unsigned int i0 = i * depthImgWidth + j;
				unsigned int i1 = (i + 1)*depthImgWidth + j;
				unsigned int i2 = i * depthImgWidth + j + 1;
				unsigned int i3 = (i + 1)*depthImgWidth + j + 1;
	...
	SimpleMesh(Vector3f lineStart, Vector3f lineEnd, float numPoints, float thickness, Vector4uc color = Vector4uc(0, 255, 0, 255))
	{
		Vector3f rayDir = lineEnd - lineStart;
		rayDir.normalize();
		float totalLength = (lineEnd - lineStart).squaredNorm();
		float step = totalLength / (numPoints - 1);

		m_vertices.resize(numPoints * 3);

		Vector3f currLocation, top, right;
		int i = 0;
		while (i < (numPoints * 3))
		{
			currLocation = lineStart + i * step * rayDir;
			...
			m_vertices[i].position = Vector4f(currLocation[0], currLocation[1], currLocation[2], 1);
			m_vertices[i++].color = color;
			m_vertices[i].position = Vector4f(top[0], top[1], top[2], 1);
			m_vertices[i++].color = color;
			m_vertices[i].position = Vector4f(right[0], right[1], right[2], 1);
			m_vertices[i++].color = color;
		}

		// Compute triangles (faces).
		m_triangles.reserve(numPoints * 3);
